...
class ControllerExtensionModuleFeedify extends Controller
{
	...
	private function handleMainPage()
	{
		...
		$data['feedify_host'] = 'https://'.$feedify_host_name;
		$data['secure_feedify_host'] = 'https://'.$feedify_host_name;
		$data['resend_email_url'] = '//'.$feedify_host_name.
			'/thirdparty/signup?action=resendVerificationEmail&licenseCode='
			.urlencode($m_license_code).'&next='.urlencode($next_url).
			'&activation_url='.urlencode($activation_url).'&channel=opencart';

		$data['module_route'] = 'module/feedify';
		$data['we_page'] = 'main';
	...
	*/
	private function feedifyUpdateFeedifyOptions()
	{
		...
		if (!empty($wlc))
		{
			...
			//$data['feedify_module'] = $feedify_settings['feedify_module'];
