...
class ModelCheckoutOrder extends Model {
	...
	public function GetOrderQuantities($orer_id){
		$query = $this->db->query("SELECT SUM(op.`quantity`) as total, op.`order_id` FROM `" . DB_PREFIX . "order_product` op GROUP BY op.`order_id`
HAVING op.`order_id` = '".$orer_id."' ");

		return $query->row;
		
		}
	public function GetOrderWeight($orer_id){
		$query = $this->db->query("SELECT SUM(p.`weight` * op.`quantity`) AS total, op.`order_id`
	...
	public function GetOrderItemDescription($orer_id){
		$query = $this->db->query("SELECT op.`name`
									FROM " . DB_PREFIX . "order_product op
									WHERE op.`order_id` = '".$orer_id."' ");
