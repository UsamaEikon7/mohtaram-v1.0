...
class ControllerProductProduct extends Controller {
	...
	public function sale(){
		...
		$this->document->setDescription("Sale");
		$this->document->setKeywords("Sale");
		...
		$data['breadcrumbs'][] = array(
			'text' => "Product Sale",
			'href' => $this->url->link('product/product/sale')
		);
		...
		if (isset($this->request->get['limit'])) {
			$url .= '&limit=' . $this->request->get['limit'];
				...
				$product_total = $this->model_catalog_product->getDiscountedTotalProducts($filter_data);
				//echo $product_total; exit;
				$results = $this->model_catalog_product->getDiscountedProducts($filter_data);
				...
				foreach ($results as $result) {
					...
					if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
						$price_original = $this->currency->format($this->tax->calculate($result['price_original'], $result['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
					} else {
				...
				foreach($limits as $value) {
					$data['limits'][] = array(
						'text'  => $value,
						'value' => $value,
						'href'  => $this->url->link('product/category', 'limit=' . $value)
					);
				}
				...
				$pagination->url = $this->url->link('product/category', 'page={page}');
